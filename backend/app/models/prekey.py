...
class OneTimePreKey(Base):
    ...
    owner's device, so a consumed row has no further use to the server, and the previous
    "used" flag meant consumed rows accumulated forever with nothing ever resetting them.
    """

    __tablename__ = "one_time_prekeys"
    __table_args__ = (UniqueConstraint("user_id", "key_id", name="uq_one_time_prekeys_user_key"),)
